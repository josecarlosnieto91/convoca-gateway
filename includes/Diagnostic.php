...
class Diagnostic {
	...
	const CACHE_KEY        = 'convoca_gateway_diagnostic_cache';
	...
	public static function fix_default_terminal(): array {
		...
		return array(
			'success' => true,
			'message' => __( 'Terminal configurado a 001', 'convoca-gateway' ),
		);
	}
	...
	public static function fix_create_pages(): array {
		...
		if ( empty( $created ) ) {
			return array(
				'success' => true,
				'message' => __( 'Las páginas ya estaban configuradas', 'convoca-gateway' ),
			);
		}

		return array(
			'success' => true,
			'message' => __( 'Creadas: ', 'convoca-gateway' ) . implode( ', ', $created ),
		);
	}
