...
class Dashboard_Widget
{
                        ...
                        <tbody>
                            <?php foreach ($data['last_7_days'] as $day): ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="bdg-bar-container" title="<?php echo esc_attr(sprintf(__('%d pagos, %s', 'convoca-gateway'), $day['count'], CPT_Pago::format_amount($day['total']))); ?>">
    ...
    private function get_dashboard_data(): array
    {
        ...
        if (empty($paid_posts)) {
            $data = [
                'total_month' => 0, 'count_today' => 0, 'count_month' => 0,
                'methods'     => ['tarjeta_pct' => 0, 'bizum_pct' => 0],
                'last_7_days' => [],
            ];
        ...
        foreach ($paid_posts as $pid) {
            $total_month += (int) get_post_meta($pid, '_bdg_amount_cents', true);
            $method = get_post_meta($pid, '_bdg_method', true);
            if (isset($methods_count[$method])) $methods_count[$method]++;

            $paid_at = get_post_meta($pid, '_bdg_paid_at', true);
            if ($paid_at && strtotime($paid_at) >= $today_start) $count_today++;
        }
        ...
        set_transient('bdg_dashboard_stats', $data, HOUR_IN_SECONDS);
