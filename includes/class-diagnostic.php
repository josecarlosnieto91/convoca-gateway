...
class Diagnostic
{
    ...
    public static function check_terminal(): array
    {
        ...
        if (empty($terminal)) {
            return self::result('terminal', 'Terminal', 'No configurado', self::SEVERITY_WARNING, 'Configura el terminal (por defecto 001)', [__CLASS__, 'fix_default_terminal']);
        }
    ...
    public static function check_return_pages(): array
    {
        ...
        if ($ok_page_id > 0) {
            ...
            if ($ok_page && $ok_page->post_status === 'publish') {
                ...
                $results['ok_page'] = self::result('ok_page', 'Página de pago OK', 'No existe o no publicada', self::SEVERITY_ERROR, 'La página configurada no existe o no está publicada', [__CLASS__, 'fix_create_pages']);
                $has_error = true;
            }
        } else {
            $results['ok_page'] = self::result('ok_page', 'Página de pago OK', 'No configurada', self::SEVERITY_ERROR, 'Crea una página con el shortcode [biodevas_pago_ok]', [__CLASS__, 'fix_create_pages']);
            $has_error = true;
        }
        ...
        if ($ko_page_id > 0) {
            ...
            if ($ko_page && $ko_page->post_status === 'publish') {
                ...
                $results['ko_page'] = self::result('ko_page', 'Página de pago error', 'No existe o no publicada', self::SEVERITY_ERROR, 'La página configurada no existe o no está publicada', [__CLASS__, 'fix_create_pages']);
                $has_error = true;
            }
        } else {
            $results['ko_page'] = self::result('ko_page', 'Página de pago error', 'No configurada', self::SEVERITY_ERROR, 'Crea una página con el shortcode [biodevas_pago_ko]', [__CLASS__, 'fix_create_pages']);
            $has_error = true;
        }
        ...
        return [
            ...
            'fix_callback' => $has_error ? [__CLASS__, 'fix_create_pages'] : null,
            'children' => $results,
        ];
    }
    ...
    public static function get_menu_badge(): array
    {
        ...
        if (!empty($cached['has_errors'])) {
            return ['severity' => 'error', 'message' => 'Hay errores de configuración'];
        }

        if (!empty($cached['has_warnings'])) {
            return ['severity' => 'warning', 'message' => 'Hay advertencias'];
        }

        return ['severity' => 'ok', 'message' => 'Todo correcto'];
    }
    ...
    public static function fix_default_terminal(): array
    {
        ...
        return ['success' => true, 'message' => 'Terminal configurado a 001'];
    }
    ...
    public static function fix_create_pages(): array
    {
        ...
        if (empty($created)) {
            return ['success' => true, 'message' => 'Las páginas ya estaban configuradas'];
        }

        return ['success' => true, 'message' => 'Creadas: ' . implode(', ', $created)];
    }
