...
class Admin_Dashboard {
	...
	public function render_widget(): void {
		...
		<style>
			.conv-gateway-stats-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 20px; }
			.conv-gateway-stat { background: #f6f7f7; padding: 15px; border-radius: 4px; border-left: 4px solid #0073aa; }
			.conv-gateway-stat-label { display: block; font-size: 12px; color: #646970; }
			.conv-gateway-stat-value { display: block; font-size: 20px; font-weight: 600; margin-top: 5px; }
			.conv-gateway-section h4 { margin: 15px 0 10px; border-bottom: 1px solid #dcdcde; padding-bottom: 5px; }
			...
			.conv-gateway-chart-table { width: 100%; border-collapse: collapse; font-size: 12px; }
			.conv-gateway-chart-table th { text-align: left; padding: 5px; background: #f6f7f7; }
			.conv-gateway-chart-table td { padding: 5px; border-bottom: 1px solid #f0f0f1; }
	...
	private function get_dashboard_data(): array {
		...
		foreach ( $paid_posts as $pid ) {
			$amount       = (int) get_post_meta( $pid, '_convoca_amount_cents', true );
			$total_month += $amount;
